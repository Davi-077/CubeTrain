...
function currentUserName() {
  return (localStorage.getItem("cubetrain_last_user") || "usuario").trim();
}
...
let cube3DPlayer = null;

function getCube3DPlayer() {
  if (cube3DPlayer) return cube3DPlayer;

  const mount = document.querySelector("#caseCubeMount");
  if (!mount) {
    throw new Error("Área do cubo 3D não encontrada.");
  }

  cube3DPlayer = new TwistyPlayer({
    puzzle: "3x3x3",
    alg: "",
    background: "none",
    controlPanel: "none",
  ...
  cube3DPlayer.style.width = "100%";
  cube3DPlayer.style.height = "360px";
  cube3DPlayer.style.display = "block";

  mount.innerHTML = "";
  mount.appendChild(cube3DPlayer);
...
function invertMoveToken(token) {
  token = token.trim();
  if (!token) return "";
  // Mantém comentários fora do algoritmo; os dados atuais já vêm sem comentários.
  if (token.endsWith("2") || token.endsWith("2'")) {
...
function configurePlayerForBuild(solutionAlg) {
  const player = getCube3DPlayer();
  const setupAlg = invertAlgorithm(solutionAlg);

  // Fase 1: parte do cubo resolvido e executa o inverso da solução.
  player.experimentalSetupAlg = "";
  player.experimentalSetupAnchor = "start";
  player.alg = setupAlg;
  try { player.tempoScale = Number(document.querySelector("#cubeSpeed").value || 1); } catch {}
  return setupAlg;
}

function configurePlayerForSolve(solutionAlg) {
  const player = getCube3DPlayer();
  const setupAlg = invertAlgorithm(solutionAlg);

  // Fase 2: começa exatamente no caso montado e então executa a solução.
...
function updateCubeStage(kind, text) {
  const badge = document.querySelector("#cubeStageBadge");
  const stageText = document.querySelector("#cubeStageText");
  badge.className = "cube-stage-badge";
  badge.classList.add(kind);
  const labels = {
    "building": "MONTANDO",
    "case-ready": "CASO PRONTO",
    "solving": "RESOLVENDO",
    "solved": "RESOLVIDO",
    "ready": "PRONTO"
  };
  badge.textContent = labels[kind] || "PRONTO";
  stageText.textContent = text;
}

async function playTwistyPlayer(player) {
  try {
    if (typeof player.pause === "function") player.pause();
    // Reiniciar recriando posição pelo algoritmo/configuração.
    if (player.timeline && typeof player.timeline.setTimestamp === "function") {
      try { player.timeline.setTimestamp(0); } catch {}
    }
    ...
    console.warn("play() não respondeu como esperado", e);
  }
  // Fallback: os controles internos podem ser reativados futuramente, mas
  // nos navegadores atuais do cubing.js play() funciona.
}

function prepareCaseAnimationButtons(solutionAlg) {
  const buildBtn = document.querySelector("#buildCaseBtn");
  const solveBtn = document.querySelector("#solveCaseBtn");
  const setupText = document.querySelector("#caseSetupAlgorithm");

  const setupAlg = invertAlgorithm(solutionAlg);
  setupText.textContent = setupAlg;
  solveBtn.disabled = true;

  configurePlayerForBuild(solutionAlg);
  updateCubeStage("ready", "Primeiro veja como o cubo resolvido chega exatamente neste caso.");

  buildBtn.onclick = async () => {
    configurePlayerForBuild(solutionAlg);
    solveBtn.disabled = false;
    updateCubeStage("building", "O cubo está fazendo o embaralhamento para chegar no caso.");
    const player = getCube3DPlayer();
    await playTwistyPlayer(player);
    updateCubeStage("case-ready", "Agora o cubo está no caso. Clique em Resolver este caso.");
  };

  solveBtn.onclick = async () => {
    configurePlayerForSolve(solutionAlg);
    updateCubeStage("solving", "Agora acompanhe o algoritmo que resolve este caso.");
    const player = getCube3DPlayer();
    await playTwistyPlayer(player);
    updateCubeStage("solved", "Pronto: este algoritmo levou o caso até o cubo resolvido.");
  };
}
...
function applyCaseAlgorithm(algorithm, item) {
  ...
  prepareCaseAnimationButtons(algorithm);
}
